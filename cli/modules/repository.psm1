...
function CheckoutBranch {
    ...
    git -C "$Repository" fetch -q | Out-Host
    if ($LASTEXITCODE -ne 0) { throw "git fetch failed. Check: 1) VPN is connected 2) Git credentials are valid (try 'Reset credentials' in DevOps tab)" }
...
function Push {
    ...
    git -C "$Repository" push -u origin "$Branch" | Out-Host
    if ($LASTEXITCODE -ne 0) { throw "git push '$Branch' failed. Check: 1) VPN connected 2) Git credentials valid 3) You have write access to '$AzDoRepository'" }
}
